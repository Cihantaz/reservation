...
def auth_login(payload: dict, db: Session = Depends(get_db)):
    ...
    sess = login_with_email(db, email=email)
...
def auth_logout_route(
    ...
    if token:
        auth_logout(db, token=token, actor_email=user.email)
        db.commit()
...
def admin_create_room(payload: RoomUpsertIn, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    room = Room(
        ...
        exam_capacity=derive_exam_capacity(payload.building, payload.class_capacity),
    )
    db.add(room)
    try:
        db.flush()
...
def admin_update_room(room_id: int, payload: RoomUpsertIn, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    room.exam_capacity = derive_exam_capacity(payload.building, payload.class_capacity)
    try:
        db.flush()
    except IntegrityError:
        db.rollback()
...
def admin_upload_rooms_save(
    ...
    for it in items:
        ...
        exam_cap = derive_exam_capacity(building, cap)
