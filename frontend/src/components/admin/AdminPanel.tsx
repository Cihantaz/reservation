...
function summarizeCourseLabel(course: { code?: string; name?: string } | null | undefined): string {
  if (!course?.code) return "Ders secilmedi";
...
function mapAuditAction(action: string): string {
  ...
  const mappings: Record<string, string> = {
    "auth.login": "Giris",
    "auth.logout": "Cikis",
    "reservation.lock": "Rezervasyon kilitleme",
    "reservation.lock.cells": "Hucre kilitleme",
    "reservation.confirm": "Rezervasyon onayi",
    "reservation.confirm.cells": "Hucre rezervasyon onayi",
    "reservation.cancel": "Rezervasyon iptali",
    "admin.schedule.preview": "Ders programi onizleme",
    "admin.schedule.save": "Ders programi kaydetme",
    "admin.rooms.preview": "Sinif listesi onizleme",
    "admin.rooms.save": "Sinif listesi kaydetme",
    "admin.global_reservations.view": "Global rezervasyon goruntuleme",
    "otp.request": "OTP talebi"
  };
  if (mappings[value]) return mappings[value];
  if (value.startsWith("admin.room.")) return "Sinif yonetimi";
  if (value.startsWith("admin.course.")) return "Ders yonetimi";
  if (value.startsWith("admin.slot.")) return "Slot yonetimi";
  if (value.startsWith("admin.rooms.")) return "Toplu sinif islemi";
  if (value.startsWith("admin.courses.")) return "Toplu ders islemi";
  if (value.startsWith("admin.slots.")) return "Toplu slot islemi";
  if (value.startsWith("admin.")) return "Admin islemi";
  return value || "Bilinmeyen islem";
}
...
export default function AdminPanel(props: { token: string }) {
  ...
  const groupedGlobalItems = useMemo<GlobalReservationGroup[]>(() => {
    const grouped = new Map<string, GlobalReservationGroup>();
    ...
    for (const item of globalItems) {
      ...
      if (!current) {
        grouped.set(key, {
          ...
          courseLabel: summarizeCourseLabel(item.course),
          roomNames: item.room?.name ? [item.room.name] : [],
          slotCodes: item.slot?.code ? [item.slot.code] : [],
          requestedCapacity: Number(item.requested_capacity ?? 0),
          cellCount: 1
        });
        continue;
      }

      if (item.room?.name && !current.roomNames.includes(item.room.name)) current.roomNames.push(item.room.name);
      if (item.slot?.code && !current.slotCodes.includes(item.slot.code)) current.slotCodes.push(item.slot.code);
      current.requestedCapacity = Math.max(current.requestedCapacity, Number(item.requested_capacity ?? 0));
      current.cellCount += 1;
    }
    ...
    return Array.from(grouped.values())
      .map((group) => ({
        ...
        slotCodes: [...group.slotCodes].sort((a, b) => a.localeCompare(b, "tr"))
      }))
      .sort((a, b) => {
        const dayDiff = new Date(b.day).getTime() - new Date(a.day).getTime();
        if (dayDiff !== 0) return dayDiff;
        return a.actorEmail.localeCompare(b.actorEmail, "tr");
      });
  }, [globalItems]);

  const groupedLogItems = useMemo<AuditLogGroup[]>(() => {
    const grouped = new Map<string, AuditLogGroup>();

    for (const item of logItems) {
      const actionLabel = mapAuditAction(item.action ?? "");
      const detailPreview = String(item.detail || item.entity || "-");
      const createdAt = item.created_at || "";
      const actorEmail = item.actor_email || "-";
      const bucketTime = createdAt ? createdAt.slice(0, 16) : "";
      const key = [actorEmail, actionLabel, detailPreview, bucketTime].join("|");
    ...
    return Array.from(grouped.values()).sort((a, b) => b.latestAt.localeCompare(a.latestAt));
  }, [logItems]);
  ...
  async function loadAuditLogs() {
    ...
    try {
      ...
      const actionQuery = logAction.trim().toLocaleLowerCase("tr-TR");
      const filteredItems = actionQuery
        ? rawItems.filter((item: any) => {
            const rawAction = String(item.action ?? "").toLocaleLowerCase("tr-TR");
            const mappedAction = mapAuditAction(String(item.action ?? "")).toLocaleLowerCase("tr-TR");
            return rawAction.includes(actionQuery) || mappedAction.includes(actionQuery);
          })
        : rawItems;
      setLogTotal(actionQuery ? filteredItems.length : res.total ?? 0);
    ...
    <div className="space-y-6">
      ...
      {tab === "rezervasyonlar" ? (
        <Card className="p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="text-base font-semibold">Global Rezervasyon Ozeti</div>
              <div className="text-sm text-white/55">Ayni kullanici, tarih, ders ve amac baglamindaki kayitlar tek satirda toplanir.</div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge tone="slate">Ozet Satir: {groupedGlobalItems.length}</Badge>
              <Badge tone="slate">Toplam Hucre: {globalTotal}</Badge>
              <Button
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            <div>
              <div className="text-sm font-semibold text-white/60">Arama</div>
              <Input value={globalQ} onChange={setGlobalQ} placeholder="E-posta / sinif / amac" />
            </div>
            <div>
              <div className="text-sm font-semibold text-white/60">Tarih</div>
              <Input value={globalDay} onChange={setGlobalDay} type="date" />
            </div>
            <div>
              <div className="text-sm font-semibold text-white/60">Durum</div>
          ...
          <div className="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-slate-950/30">
            <div className="hidden grid-cols-[220px_150px_1.1fr_1fr_1.2fr_120px_110px] gap-3 border-b border-white/10 px-4 py-3 text-sm font-semibold text-white/60 lg:grid">
              <div>Kullanici</div>
              <div>Tarih</div>
              <div>Ders / Amac</div>
              <div>Slotlar</div>
            ...
            {groupedGlobalItems.map((item) => (
              <div key={item.key} className="border-b border-white/10 px-4 py-4 last:border-b-0">
                <div className="grid gap-4 lg:grid-cols-[220px_150px_1.1fr_1fr_1.2fr_120px_110px] lg:items-start">
                  <div className="text-sm font-semibold text-white/85">{item.actorEmail}</div>
                  <div className="text-sm text-white/80">{item.day}</div>
                  <div className="space-y-1 text-sm text-white/80">
                    <div className="font-semibold">{item.courseLabel}</div>
                    <div className="text-white/60">{item.purpose}</div>
                  </div>
                  <div className="text-sm text-white/80">{item.slotCodes.join(", ") || "-"}</div>
                  <div className="text-sm text-white/80">{item.roomNames.join(", ") || "-"}</div>
                  <div className="space-y-1 text-sm text-white/80">
                    <div className="font-semibold">{item.requestedCapacity}</div>
                    <div className="text-white/55">Hucre: {item.cellCount}</div>
                  </div>
                  <div>
                    <Badge tone={item.status === "confirmed" ? "green" : "slate"}>{item.status === "confirmed" ? "Onayli" : "Iptal"}</Badge>
                  </div>
                </div>
              </div>
            ))}
            {groupedGlobalItems.length === 0 ? <div className="px-4 py-6 text-sm text-white/60">Kayit yok.</div> : null}
          </div>

          <div className="mt-4 flex items-center justify-between">
            <div className="text-sm text-white/50">Sayfa: {Math.floor(globalOffset / 50) + 1}</div>
            <div className="flex gap-2">
              <Button
                variant="secondary"
      ...
      {tab === "loglar" ? (
        <Card className="p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="text-base font-semibold">Audit Log Ozeti</div>
              <div className="text-sm text-white/55">Benzer loglar ayni kullanici ve islem bazinda tek satirda toplanir.</div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge tone="slate">Ozet Satir: {groupedLogItems.length}</Badge>
              <Badge tone="slate">Toplam Kayit: {logTotal}</Badge>
              <Button
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            <div>
              <div className="text-sm font-semibold text-white/60">Kullanici E-posta</div>
              <Input value={logActor} onChange={setLogActor} placeholder="cihan.tazeoz@isikun.edu.tr" />
            </div>
            <div>
              <div className="text-sm font-semibold text-white/60">Aksiyon</div>
              <Input value={logAction} onChange={setLogAction} placeholder="Giris / rezervasyon / admin" />
            </div>
          ...
          <div className="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-slate-950/30">
            <div className="hidden grid-cols-[180px_220px_220px_1fr_90px] gap-3 border-b border-white/10 px-4 py-3 text-sm font-semibold text-white/60 lg:grid">
              <div>Tarih / Saat</div>
              <div>Kullanici</div>
              <div>Aksiyon</div>
              <div>Detay</div>
            ...
            {groupedLogItems.map((item) => (
              <div key={item.key} className="border-b border-white/10 px-4 py-4 last:border-b-0">
                <div className="grid gap-4 lg:grid-cols-[180px_220px_220px_1fr_90px] lg:items-start">
                  <div className="text-sm text-white/70">{item.latestAt ? new Date(item.latestAt).toLocaleString("tr-TR") : "-"}</div>
                  <div className="text-sm font-semibold text-white/85">{item.actorEmail}</div>
                  <div className="text-sm text-white/80">{item.actionLabel}</div>
                  <div className="text-sm text-white/65">{item.detailPreview}</div>
                  <div><Badge tone="slate">{item.count}</Badge></div>
                </div>
              </div>
            ))}
            {groupedLogItems.length === 0 ? <div className="px-4 py-6 text-sm text-white/60">Kayit yok.</div> : null}
          </div>

          <div className="mt-4 flex items-center justify-between">
            <div className="text-sm text-white/50">Sayfa: {Math.floor(logOffset / 50) + 1}</div>
            <div className="flex gap-2">
              <Button
                variant="secondary"
